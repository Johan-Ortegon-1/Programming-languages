...
def to_be_visited():
  ...
  for index in range(number_of_vertices):
      #  Si el vertice no ha sido visitado Y (estoy en la prime iteracion O la distancia del indice donde estoy es menor al menor que tenia antes)
...
visited_and_distance = [[0, 0]]  # First iteration
...
for vertex in range(number_of_vertices):
  ...
  for neighbor_index in range(number_of_vertices):
    ...
    # Hay un camino directo entre to_visit y neighbor_index Y No lo he visitado aun
    if vertices[to_visit][neighbor_index] == 1 and \
     visited_and_distance[neighbor_index][0] == 0:
        #La nuevo distancia es la suma de lo que me cuesta llegar al nodo to_visit + el peso que lo conecta con neighbor_index
